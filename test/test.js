...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof sminsorted, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( sminsorted.length, 1, 'has expected arity' );
	t.end();
});
...
tape( 'the function calculates the minimum value of a one-dimensional ndarray', function test( t ) {
	...
	v = sminsorted( [ vector( x, 2, 1, 0 ) ] );
	t.strictEqual( v, -5.0, 'returns expected value' );

	x = new Float32Array( [ -0.0, -0.0, 0.0 ] );
	v = sminsorted( [ vector( x, 3, 1, 0 ) ] );
